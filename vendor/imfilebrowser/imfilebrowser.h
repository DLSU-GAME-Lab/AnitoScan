...
enum ImGuiFileBrowserFlags_ : std::uint32_t
{
    ...
    ImGuiFileBrowserFlags_Embedded              = 1 << 12, // allow user to whether display the directory options or not
};
...
inline void ImGui::FileBrowser::Display()
{
    ...
    //if (flags_ & ImGuiFileBrowserFlags_Embedded) {
    ...
    if (flags_ & ImGuiFileBrowserFlags_Embedded) {
        if (!BeginChild(openLabel_.c_str(), ImVec2(0, 0), ImGuiChildFlags_None,
 ...
 //   ScopeGuard endPopup([] { EndPopup(); });
   ...
   // isOpened_ = false;
    ...
    //else
    //{
    //    // keep original modal behavior untouched
    //    if (shouldOpen_)
    //        OpenPopup(openLabel_.c_str());
    ...
    //    if (!BeginPopupModal(openLabel_.c_str(), nullptr,
    //        flags_ & ImGuiFileBrowserFlags_NoTitleBar ? ImGuiWindowFlags_NoTitleBar : 0))
    //        return;
    //}
    ...
    if(!(flags_ & ImGuiFileBrowserFlags_Embedded) && (flags_ & ImGuiFileBrowserFlags_CreateNewDir))
    {
        SameLine();
        if(SmallButton("+"))
        {
...

    {
        ...
        for(unsigned int rscIndex = 0; rscIndex < fileRecords_.size(); ++rscIndex)
        {
            ...
            //edit
            if ((flags_ & ImGuiFileBrowserFlags_Embedded) && rsc.name == "..") {
                continue;
            }
...
inline void ImGui::FileBrowser::Refresh() {
    UpdateFileRecords();
}
